...
extension Notification.Name {
    static let hyperviewPinsChanged = Notification.Name("hyperview.pinsChanged")
    /// userInfo: ["id": UUID] — switch to Notes, then open the note.
    static let hyperviewRevealNote = Notification.Name("hyperview.revealNote")
    /// userInfo: ["id": String] — switch to Reminders, then select it.
    static let hyperviewRevealReminder = Notification.Name("hyperview.revealReminder")
}
...
enum PinStore {
    ...
    static func toggle(note id: UUID) {
        ...
        NotificationCenter.default.post(name: .hyperviewPinsChanged, object: nil)
    }
...
struct PinnedNotesCard: View {
    ...
    var body: some View {
        ...
        if !visible.isEmpty {
                    ...
                    ForEach(visible) { note in
                        Button {
                            NotificationCenter.default.post(
                                name: .hyperviewRevealNote, object: nil, userInfo: ["id": note.id]
                            )
                        } label: {
            ...
            .onReceive(NotificationCenter.default.publisher(for: .hyperviewPinsChanged)) { _ in
...
struct PinnedRemindersCard: View {
    ...
    private func row(_ reminder: ReminderSnapshot) -> some View {
        Button {
            NotificationCenter.default.post(
                name: .hyperviewRevealReminder, object: nil, userInfo: ["id": reminder.id]
            )
        } label: {
