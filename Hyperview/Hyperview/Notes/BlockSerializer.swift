...
//  TipTap/ProseMirror document and Hyperview's `[Block]` store. Built and tested
//  FIRST, in isolation from the WKWebView editor (see HyperviewTests).
//
...
//      over the blocks Hyperview itself produces (round-trip == identity).
//
//  Known v1 flattening (acceptable, documented): a multi-paragraph blockquote or
//  a nested list collapses to a single inline run. Hyperview never emits those,
